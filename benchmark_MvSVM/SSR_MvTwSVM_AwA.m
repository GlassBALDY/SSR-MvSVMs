...
global_options = optimoptions('quadprog');
global_options.OptimalityTolerance = 1e-14;
global_options.ConstraintTolerance = 1e-14;
global_options.FunctionTolerance = 1e-14;
global_options.StepTolerance = 1e-14;
global_options.MaxIterations = 10000;
...
%% Load AwA data set
load('..\Data\AwA_45_200.mat')
for idx_task = 1 : 45
    comm = sprintf('%s%d', 'task_temp = task_', idx_task);
    eval(comm)
    name = task_temp.name;
    XA = task_temp.XA; XB = task_temp.XB; y = task_temp.y;
    ...
    for idx_fold = 1 : k_fold_num
        ...
        [SSR_table] = SSR_search_MvTwSVM(trainXA, trainXB, trainy, testXA, testXB, testy, ...
            ...
            'directory', ['SSR_MvTwSVM_AwA\AwA_', num2str(idx_task), '\'], 'result_name', ['fold_', num2str(idx_fold)]);
    end
end
...
data_dir = dir('.\SSR_MvTwSVM_AwA\');
data_name = {data_dir.name};
data_name = data_name(3: end)';
results_dir = '.\SSR_MvTwSVM_AwA\';
