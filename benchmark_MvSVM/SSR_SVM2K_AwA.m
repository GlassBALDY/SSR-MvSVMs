clear all
addpath('..\Methods\', '..\Methods\SVM_2K\');
...
for idx_task = 1: 45
    ...
    for idx_fold = 1:k_fold_num
        ...
        [SSR_table] = SSR_search_SVM2K(trainXA, trainXB, trainy, testXA, testXB, testy, ...
            Box_Cons_series, D_series, ...
            kernelA, kernelparamA, kernelB, kernelparamB, epsilon, ...
            'directory', ['SSR_SVM2K_AwA\AwA_', num2str(idx_task), '\'], 'result_name', ['fold_', num2str(idx_fold)]);
    end
end

%% extract the SSR_tables
%% SSR-SVM-2K's result extractor
% set the directory of results
data_dir = dir('.\SSR_SVM2K_AwA\');
data_name = {data_dir.name};
data_name = data_name(3: end)';
results_dir = '.\SSR_SVM2K_AwA\';
