function [SSR_table, diagnosis] = SSR_comparison_MvTwSVM(trainXA, trainXB, trainy, testXA, testXB, testy, ...
...
% =========== main function =====================================
    ...
    % via gurobi
    mvtwsvmmodel_gurobi = MvTwsvmtrain_gurobi(trainXA, trainXB, trainy, Cp, Cn, D, kerneltypeA, kernelparamA, kerneltypeB, kernelparamB, ...
        epsilon, 'Tikhonov', Tikhonov);
    [ ~, ~, ~, ~, ~, ~, ~, mvtwsvmmodel_gurobi] = MvTwsvmpredict(mvtwsvmmodel_gurobi, testXA, testXB, testy);

    % record Accuracy
    Acc(1, 2) = mvtwsvmmodel_gurobi.accuracy;
    Acc_viewA(1, 2) = mvtwsvmmodel_gurobi.accuracy_viewA;
    Acc_viewB(1, 2) = mvtwsvmmodel_gurobi.accuracy_viewB;
    ...
    Time(1, 2) = mvtwsvmmodel_gurobi.training_time;
    ...
    for i_step = 2: L
        ...
        %% SSR-MvTwSVM (via SSR with quadprog)
        ...
        %% Raw-MvTwSVM (via gurobi)
        raw_mvtwsvmmodel = MvTwsvmtrain_gurobi(trainXA, trainXB, trainy, Cp, Cn, D, kerneltypeA, kernelparamA, kerneltypeB, kernelparamB, epsilon);
        %用predict函数计算模型准确度
