function [ ] = myMvsvmplot(model, definition, plot_name)
...
% - Jiayi Zhu, 15th February 2022, Adding scaledMvTHSVM
...
% plot the graphic
    ...
    if strcmp(name, 'SVM+')        % when name = 'SVM+'
        ...
        % plot decision plane
        contour(Ax, Ay, svmplusscoreGrid, [1 1], 'ShowText', 'on', 'color', 'r', 'Linestyle', '--');
        contour(Ax, Ay, svmplusscoreGrid, [-1 -1], 'ShowText', 'on', 'color', 'b', 'Linestyle', '--');
        contour(Ax, Ay, svmplusscoreGrid, [0 0], 'ShowText', 'on', 'color', 'k');
        ...
        %%
    elseif strcmp(name, 'SVM-2K')||strcmp(name, 'SVM-2K fast algorithm')   % when name = 'SVM-2K'
        % import parameters from model
        XA = model.CA;
        XB = model.CB;
        ...
        % plot epsilon-vector
        eps_A = XA(model.index_beta_in_epsilon, :);
        plot(eps_A(:, 1), eps_A(:, 2), 'm^', 'LineWidth', 1.2, 'markersize', 8);
        ...
        % plot color band
        svm2kscoreA_L = svm2kscoreA;
        svm2kscoreA_L(svm2kscoreA_L < 1 & svm2kscoreA_L > -1) = 0;
        svm2kscoreA_L(svm2kscoreA_L > 1 | svm2kscoreA_L < -1) = nan;
        ...
        %         legend('positive sample', 'negative sample', '\bf{\alpha}_{A} = 0', '\epsilon-samples', '+ decision plane', '-decision plane', 'decision plane', '+1 test sample', '-1 test sample', 'Location', 'NorthEastOutside');
        ...
        % plot epsilon-vector
        eps_B = XB(model.index_beta_in_epsilon, :);
        plot(eps_B(:, 1), eps_B(:, 2), 'm^', 'LineWidth', 1.2, 'markersize', 8);
        ...
        %         legend('positive sample', 'negative sample', '\bf{\alpha}_{B} = 0', '\epsilon-samples', '+ decision plane', '- decision plane', 'decision plane', '+1 test sample', '-1 test sample', 'Location', 'NorthEastOutside');

        hold off
        %%
    elseif strcmp(name, 'SVM-2K\_SSR') || strcmp(name, 'SVM-2K\_noSSR')   % when name = 'SVM-2K_SSR'
        % import parameters from model
        XA = model.CA;
        XB = model.CB;
        xp = model.CA(model.y == 1, :);
        ...
        hold off
        %%
    elseif strcmp(name, 'MCPK')   % when name = 'MCPK'
        % import parameters from model
        XA = model.CA;
        XB = model.CB;
        xp = model.CA(model.y == 1, :);
        ...
        % mesh viewA and viewB's grid
        [Ax, Ay] = meshgrid(min(XA(:, 1)) : h: max(XA(:, 1)) + 8, min(XA(:, 2)): h: max(XA(:, 2)));
        [Bx, By] = meshgrid(min(XB(:, 1)) : h: max(XB(:, 1)) + 8, min(XB(:, 2)): h: max(XB(:, 2)));
        [~, MCPKsvmscoreA, ~] = predict(model, [Ax(:), Ay(:)], []);
        [~, ~, MCPKsvmscoreB] = predict(model, [], [Bx(:), By(:)]);
        MCPKsvmscoreAGrid = reshape(MCPKsvmscoreA(:, 1), size(Ax, 1), size(Ay, 2));
        MCPKsvmscoreBGrid = reshape(MCPKsvmscoreB(:, 1), size(Bx, 1), size(By, 2));
        ...
        % plot viewA's decision plane
        contour(Ax, Ay, MCPKsvmscoreAGrid, [1 1], 'ShowText', 'on', 'color', 'r', 'Linestyle', '--');
        contour(Ax, Ay, MCPKsvmscoreAGrid, [-1 -1], 'ShowText', 'on', 'color', 'b', 'Linestyle', '--');
        contour(Ax, Ay, MCPKsvmscoreAGrid, [0 0], 'ShowText', 'on', 'color', 'k');
        ...
        legend('positive sample', 'negative sample', '+ decision plane', '- decision plane', 'decision plane', '+1 test sample', '-1 test sample', 'Location', 'NorthEastOutside');
        ...
        contour(Bx, By, MCPKsvmscoreBGrid, [1 1], 'ShowText', 'on', 'color', 'r', 'Linestyle', '--');
        contour(Bx, By, MCPKsvmscoreBGrid, [-1 -1], 'ShowText', 'on', 'color', 'b', 'Linestyle', '--');
        contour(Bx, By, MCPKsvmscoreBGrid, [0 0], 'ShowText', 'on', 'color', 'k');
    ...
    elseif ( strcmp(name, 'MvTHSVM') || strcmp(name, 'MvTHSVM2C') ...
            ...
            || strcmp(name, 'scaled-MvTHSVM'))	% when name = 'MvTHSVM'
        % import parameters from model
        % import train set
        XA = model.CA;
        XB = model.CB;
        ...
        % plot viewA's decision plane
        contour(Ax, Ay, MvTHSVMscoreApGrid, [1 1], 'color', 'r', 'Linestyle', '--');
        contour(Ax, Ay, MvTHSVMscoreAnGrid, [1 1], 'color', 'b', 'Linestyle', '--');
        contour(Ax, Ay, MvTHSVMscoreAnGrid - MvTHSVMscoreApGrid, [0 0], 'color', 'k', 'LineWidth',1.5);
        
        % plot viewA's color block
        % draw region in the + sphere
%         MvTHSVMscoreApGrid_color = MvTHSVMscoreApGrid;
%         MvTHSVMscoreApGrid_color(MvTHSVMscoreApGrid_color < 1) = 0.1;
%         MvTHSVMscoreApGrid_color(MvTHSVMscoreApGrid_color > 1) = nan;
%         contourf(Ax, Ay, MvTHSVMscoreApGrid_color, [1, 1]);
%         
%         % draw region out the + sphere
%         MvTHSVMscoreApGrid_color = MvTHSVMscoreApGrid;
%         MvTHSVMscoreApGrid_color(MvTHSVMscoreApGrid_color > 1) = 1.2;
%         MvTHSVMscoreApGrid_color(MvTHSVMscoreApGrid_color < 1) = nan;
%         contourf(Ax, Ay, MvTHSVMscoreApGrid_color, [1, 1]);
...
%         figure               % create a layer
        ...
        % plot viewB's decision plane
        contour(Bx, By, MvTHSVMscoreBpGrid, [1 1], 'color', 'r', 'Linestyle', '--');
        contour(Bx, By, MvTHSVMscoreBnGrid, [1 1], 'color', 'b', 'Linestyle', '--');
...
%         legend('positive sample', 'negative sample', 'positive support vector', 'negative support vector', '+ decision plane', '- decision plane', 'decision plane', 'positive test sample', 'negative test sample', 'Location','NorthEastOutside');
    ...
    elseif strcmp(name, 'Coupling-MvTHSVM')	% when name = 'Coupling-MvTHSVM'
        % import parameters from model
        % import train set
        XA = model.CA;
        XB = model.CB;
        ...
        [Bx, By] = meshgrid(min(XB(:, 1)) : h: max(XB(:, 1)), min(XB(:, 2)): h: max(XB(:, 2)));
        % calculate viewA/B's grid values
        [~, CoupTHSVMscoreAp, CoupTHSVMscoreBp, CoupTHSVMscoreAn, CoupTHSVMscoreBn] = predict(model, [Ax(:), Ay(:)], [Bx(:), By(:)]);
        % mesh viewA's grid
        CoupTHSVMscoreApGrid = reshape(CoupTHSVMscoreAp(:, 1), size(Ax, 1), size(Ay, 2));
        CoupTHSVMscoreAnGrid = reshape(CoupTHSVMscoreAn(:, 1), size(Ax, 1), size(Ay, 2));
        % mesh viewB's grid
        CoupTHSVMscoreBpGrid = reshape(CoupTHSVMscoreBp(:, 1), size(Bx, 1), size(By, 2));
        CoupTHSVMscoreBnGrid = reshape(CoupTHSVMscoreBn(:, 1), size(Bx, 1), size(By, 2));
        ...
        % plot viewA's decision plane
        contour(Ax, Ay, CoupTHSVMscoreApGrid, [1 1], 'ShowText', 'on', 'color', 'r', 'Linestyle', '--');
        contour(Ax, Ay, CoupTHSVMscoreAnGrid, [1 1], 'ShowText', 'on', 'color', 'b', 'Linestyle', '--');
        contour(Ax, Ay, CoupTHSVMscoreAnGrid - CoupTHSVMscoreApGrid, [0 0], 'ShowText', 'on', 'color', 'k', 'LineWidth',1.5);
        ...
        % plot viewB's decision plane
        contour(Bx, By, CoupTHSVMscoreBpGrid, [1 1], 'ShowText', 'on', 'color', 'r', 'Linestyle', '--');
        contour(Bx, By, CoupTHSVMscoreBnGrid, [1 1], 'ShowText', 'on', 'color', 'b', 'Linestyle', '--');
        contour(Bx, By, CoupTHSVMscoreBnGrid - CoupTHSVMscoreBpGrid, [0 0], 'ShowText', 'on', 'color', 'k', 'LineWidth',1.5);
        ...
        %%
    elseif strcmp(name, 'Multi-view Twin SVM')	% when name = 'Multi-view Twin SVM'
        % import parameters from model
        % import train set
        XA = model.CA;
        XB = model.CB;
        ...
        legend('positive sample', 'negative sample', '+ decision plane', '- decision plane', 'decision plane', 'positive test sample', 'negative test sample', 'Location','NorthEastOutside');
        hold off
    elseif strcmp(name, 'MvTwSVM\_SSR') || strcmp(name, 'MvTwSVM\_noSSR')	% when name = 'Multi-view Twin SVM'
        % import parameters from model
        % import train set
        XA = model.CA;
        XB = model.CB;
