function [SSR_table, diagnosis] = SSR_comparison_SVM2K(trainXA, trainXB, trainy, testXA, testXB, testy, ...
...
% =========== main function =====================================
    ...
    dir_ = ['SSR-SVM2K-comparison_result', '_', exe_date];
    
    result_name = 'SSR-SVM2K-comparison_table';
    ...
    % via gurobi
    svm2kmodel_gurobi = svm2ktrain_gurobi(trainXA, trainXB, trainy, D, C, kerneltypeA, kernelparamA, kerneltypeB, kernelparamB, epsilon);
    [ ~, ~, ~, svm2kmodel_gurobi] = svm2kpredict(svm2kmodel_gurobi, testXA, testXB, testy);
    
    % record Accuracy
    Acc(1, 2) = svm2kmodel_gurobi.accuracy;
    Acc_viewA(1, 2) = svm2kmodel_gurobi.accuracy_viewA;
    Acc_viewB(1, 2) = svm2kmodel_gurobi.accuracy_viewB;
    ...
    Time(1, 2) = svm2kmodel_gurobi.training_time;
    ...
    for i_step = 2: L
        ...
        %% SSR-SVM-2K (via SSR with quadprog)
        [svm2kmodel_next, detailed] = DVI_rules_4_SVM2K(svm2kmodel, [C', D(1, 1)], global_options);
        %用predict函数计算模型准确度
        [ ~, ~, ~, svm2kmodel_next] = svm2kpredict(svm2kmodel_next, testXA, testXB, testy);
        
        %% Raw-SVM-2K (via gurobi)
        raw_svm2kmodel = svm2ktrain_gurobi(trainXA, trainXB, trainy, D, C, kerneltypeA, kernelparamA, kerneltypeB, kernelparamB, epsilon);
        %用predict函数计算模型准确度
